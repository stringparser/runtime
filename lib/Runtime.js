...
Runtime.prototype.stack = function(/* functions..., props */){
  ...
  function composer(host, handle, next){
    ...
    return tick(stack, {
      ...
      onError: stack.done || self.onHandleError,
      onHandle: stack.onHandle || self.onHandle
    });
  }
  ...
  function tick(stack, site){
    ...
    site.next = ++site.index < length;

    util.asyncDone(function runner(){
      site.onHandle.call(self, next, handle, stack);
      var result = handle.apply(site.context, args);
      if(!next.wait && site.next){ tick(stack, site); }
      return result;
    }, next);
    ...
    function next(err){
      ...
      if(err instanceof Error){
        site.onError.call(self, err, next, handle, stack);
        return self;
