...
Runtime.prototype.stack = function (/* sites..., props */) {
  ...
  var props = util.merge(new self.Stack(), this.constructor.prototype, this,
    util.type(sites[sites.length - 1]).plainObject && sites.pop()
  );

  composer.stack = util.merge(new this.Stack(sites), util.clone(props, true));

  function composer (host, next) {
    var proto = util.merge(new self.Stack(), util.clone(props, true));
