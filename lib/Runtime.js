...
Runtime.prototype.stack = function(/* functions..., props */){
  ...
  function composer(host, handle, next){
    var stack = new self.Stack(fns, props);
    ...
    if(typeof arguments[arguments.length-1] === 'function'){
      stack.done = util.before(2, stack.host && next || stack.args.pop());
    }

    stack.onHandle = stack.onHandle || self.onHandle;
    stack.onHandleError = stack.done || self.onHandleError;

    return tick(stack, {
      index: 0,
      queue: fns.length,
      context: stack.context || self
    });
  }

  // runs each handle
  function tick(stack, site){
    var handle = fns[site.index];
    var args = handle.stack instanceof self.Stack
      && [stack, handle, next]
      || [next].concat(stack.args);

    next.wait = Boolean(stack.wait);
    site.next = ++site.index < fns.length;

    util.asyncDone(function onNext(){
      stack.onHandle.call(self, next, handle, stack);
      var result = handle.apply(site.context, args);
      if(next.wait){ return result; }
      if(site.next){ tick(stack, site); }
      return result;
    }, next);

    function next(err){
      if(err instanceof Error){
        stack.onHandleError.call(self, err, next, handle, stack);
        return self;
      ...
      if(!site.queue && stack.done){
        stack.done.apply(self, [null].concat(stack.args));
      }
    }
