...
Runtime.prototype.stack = function(/* functions..., props */){
  ...
  if(!util.areFunctions(fns)){
    throw new TypeError('stack is empty: it has no functions in it');
  }

  var self = this, length = fns.length;
  var props = util.type(arguments[arguments.length-1]).plainObject || {};
  composer.stack = new self.Stack(fns, util.merge(props, self));

  function composer(next, site, host){
    var stack = util.merge(composer.stack.clone(), fns);
    var sites = {next: 0, count: length};
    ...
    if(site === composer){ // we are inside other stack
      ...
      if(typeof arguments[arguments.length-1] === 'function'){
        sites.done = stack.args.pop();
      }
    }

    sites.ctxt = stack.context || self;
    sites.onError = sites.done || stack.onHandleError || self.onHandleError;
  ...
  function tick(stack, sites){
    ...
    util.asyncDone(function callsite(){
      ...
      var result = site.apply(sites.ctxt, args);
