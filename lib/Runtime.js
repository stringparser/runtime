...
// missing docs
Runtime.prototype.tree = function (self) {
  var tree = { label: '', nodes: [] };

  if (self instanceof Runtime && this instanceof Stack) {
    var stack = new self.Stack(null, self);

    for (var i = 0, l = this.length; i < l; ++i) {
      stack.reduceStack.call(self, stack, this[i], i, this);

      var site = stack[i];
      if (!site || !site.fn) { return; }

      if (site.fn.stack instanceof Stack) {
        site = site.fn.stack.tree(self);
      } else {
        site = { label: site.label || site.fn.displayName || site.fn.name };
      }
