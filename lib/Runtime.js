...
Runtime.prototype.stack = function(/* sites..., props */) {
  ...
  function composer(/* arguments */) {
    ...
    stack.args = length && __slice.call(arguments) || [];
    if(length && typeof arguments[length - 1] === 'function'){
  ...
  function tick(stack){
    ...
    if(site.fn.stack instanceof Stack){
      site.context = stack;
    } else if(!site.context){
      site.context = stack.context || site;
    }

    util.asyncDone(function asyncStart(){
      stack.onHandle(site, stack);
      var result = site.fn.apply(site.context, args);
    ...
    function asyncEnd(error){
      if(error){
        (stack.onStackEnd || stack.onHandleError)(error, site, stack);
        return;
