...
Runtime.prototype.stack = function(/* functions..., props */){
  ...
  function composer(/* arguments..., callback */){
    ...
    var stack = util.reduce(funcs, self.reduceStack, clone, self);
    var sites = {next: 0, count: stack.length, length: stack.length};

    sites.args = [].slice.call(arguments);
    if(typeof arguments[arguments.length-1] === 'function'){
      sites.done = sites.onError = sites.args.pop();
    } else {
      sites.onError = stack.onHandleError || self.onHandleError;
    }

    sites.context = stack.context || self.context || self;
  ...
  function tick(stack, sites){
    ...
    next.wait = Boolean(props.wait);
    sites.next = ++sites.next < sites.length && sites.next;
    ...
    function next(err){
      ...
      if(!sites.count && sites.done){
        sites.done.apply(self, [null].concat(sites.args));
      }
