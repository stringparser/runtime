...
Runtime.prototype.stack = function(/* functions..., props */){
  var sites = [].slice.call(arguments);
  var props = util.type(sites[sites.length-1]).plainObject && sites.pop();
  if(!sites.length){ throw new TypeError('the stack is empty'); }
  ...
  function composer(next, handle, host){
    var stem = {next: 0};
    var clone = util.merge(composer.stack.clone(), {sites: []});
    var stack = util.reduce(sites, self.reduceStack, clone, self);
    stem.length = stem.count = stack.sites.length;
    ...
    if(handle === composer){ // we are inside other stack
      ...
      if(typeof arguments[arguments.length-1] === 'function'){
        stem.done = stack.args.pop();
      }
    }

    stem.ctxt = stack.context || self;
    stem.onError = stem.done || stack.onHandleError || self.onHandleError;
    stem.onHandle = stack.onHandle || self.onHandle;
  ...
  function tick(stack, stem){
    var site = stack.sites[stem.next];
    var args = site.stack instanceof self.Stack
      && [next, site, stack]
      || [next].concat(stack.args);

    next.wait = Boolean(stack.wait);
    stem.next = ++stem.next < stem.length && stem.next;

    util.asyncDone(function callsite(){
      stem.onHandle.call(self, next, site, stack);
      var result = site.apply(stem.ctxt, args);
      if(stem.next && !next.wait){ tick(stack, stem); }
      return result;
    }, next);

    function next(err){
      if(err instanceof Error){
        return stem.onError.call(self, err, next, site, stack);
      ...
      stem.onHandle.call(self, next, site, stack);
      if(stem.next){ return tick(stack, stem); }
      if(!stem.count && stem.done){
        stem.done.apply(self, [null].concat(stack.args));
      }
    }
...
Runtime.prototype.reduceStack = function(stack, site){
  if(typeof site === 'function'){ stack.sites.push(site); }
  return stack;
};
