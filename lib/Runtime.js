...
Runtime.prototype.stack = function(/* functions..., props */){
  var self = this, last = arguments[arguments.length-1];
  var props = util.merge({}, self, util.type(last).plainObject);
  ...
  function composer(next, handle, host){
    var stack = util.clone(props, true);
  ...
  function tick(stack){
    ...
    function next(err){
      if(err instanceof Error){
        return stack.onHandleError.call(self, err, next, stack);
      } else if((next.wait || next.repeat) && arguments.length){
        util.mapPush(stack.args, arguments, err ? -1 : 0);
      }

      if(next.repeat){ stack.index = index; } else {
        --stack.count; next.end = true;
        if(stack.index < stack.resumeIndex){
          stack.index = stack.resumeIndex;
        }
        self.onHandle(next, stack);
      }

      if(stack.next || next.repeat){ return tick(stack); }
      if(stack.callback && !stack.count){
...
will call some methods during the stack execution. These methods aim are to inspect, maintain bookeeping if necesary and catch errors.
