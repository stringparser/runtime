...
var debug = {
      set : require('debug')('cmd:set'),
      get : require('debug')('cmd:get'),
    parse : require('debug')('cmd:parse'),
   handle : require('debug')('cmd:handle')
};
...
function Command(options){
   ...
   */
   ...
   this.set = function (stems, options){
    ...
    if( optsIs.function ){
      opts = { handle : options };
    } else if( optsIs.object ){
      opts = util.merge({ }, options);
    }
    ...
    stems.forEach(function(stem, index){
      ...
      if( !node.children[stem] ){
        ...
        node.children[stem] = {
          _name : stem, _depth : node._depth+1
        };

        if( index > 1 ){
          node.children[stem]._parent = [
            node._parent, node._name
          ].join(' ');
        } else {
          node.children[stem]._parent = node._name;
        }
      }
   ...
   * @return this
   */
  debug.config = require('debug')('cmd:config');
  this.config = function (obj, value){

    var copy, objIs = util.type(obj);

    debug.config('objIs', objIs);
    if( objIs.undefined ){
