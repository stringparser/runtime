...
function fetch(cli){
  ...
  assert( util.isObject(cli) );

  command = JSON.parse(cli, function(key, value){
    if(value.test(/^function/))
...
function clone(fileName){
  ...
  assert( util.isString(fileName) );

  var site = path.resolve(
    // called from runtime.target
    // that is 3 calls from here:
    //   - 1st from the file runtime.target()
    //   - 2nd from the file runtime.target is defined
    //   - 3rd from this file
    path.dirname(stack[2].getFileName()), fileName
  );

  var ws = fs.createWriteStream(site);

  ws.write(
    JSON.stringify(command, function(key, value){
