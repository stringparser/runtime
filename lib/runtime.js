 ...
 * and a next callback
 *
 * returns this.consume(ctx, args, next)
 */


Runtime.prototype.next = function(/* arguments */){
  var self = this;
  var line = arguments[0], ctx = { };
  ...
  function next(stem){
    ...
    // same command with && without new arguments
    if(arguments.length){
      ctx.args = util.args(arguments).concat(next);
    }
    // continue?
    ctx.index += ctx.cmd._depth;
    ctx.done = !ctx.argv[ctx.index];
