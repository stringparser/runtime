...
function pathCompletion(partial){
  ...
  //
  partial = (partial || ['file.ext'])[0].trim();
  var notDir = !!path.extname(partial);
  if( notDir ){  return _path;  }
  var scanned = _path.filter(function(elem){
    return elem.indexOf(partial) === 0;
  }).length;
  if( scanned ){ return _path; }

  try {
    var lstat = fs.lstatSync(path.join(cwd, partial));
    if( !lstat.isDirectory() ){  return _path;  }
    partial = partial.substring(0, partial.length-1);
  } catch(err){ return _path; }
  // or didn't exist or wasn't a directory
  // not useful to throw here

  var notEmpty = false;
  fs.readdirSync(path.join(cwd, partial))
    .forEach(function(pathname){
      if( _path.indexOf(pathname) > -1 ){  return ;  }
      _path.push(path.join(partial, pathname));
      notEmpty = true;
    });
  // keep low the _path array
  if( notEmpty ){ _path.splice(_path.indexOf(partial), 1); }
  return _path;
}
/**
 *
