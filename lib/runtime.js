...
function Runtime(name, config){
  ...
  var self = this, input, output;

  config = util.type(config).plainObject || { };
  config.timer = { };
  config.repl = config.repl || true;
  config.startup = config.startup || true;
  ...
  if( config.repl ){
    ...
    this.input.on('keypress', on.keypress.bind(this));
    sink = pathCompletion[0] || on.path('.', pathCompletion);
  }

  // wire things up
  this.on('line', on.line.bind(this));
  this.on('wire', on.wire.bind(this));
  this.on('next', on.next.bind(this));
