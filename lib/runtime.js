...
Runtime.prototype.consumer = function(argv, args){
  ...
  debug(cmd._name, 'argv', argv );
  debug('done?', done, 'usesNext?', usesNext);

  if( done ){
    args.time = process.hrtime(args.hrtime || process.hrtime());
    args.time = util.prettyTime(args.time);
  ...
  if( cmd.handle ){
    ...
    debug(cmd._name, 'called with', argv);
    cmd.handle.call(scope, argv, args, next);
    if( !usesNext && cmd._depth ){  next();  }
  }

  function next(_argv){
    argv = _argv || argv;
    if( _argv ){  done = !(self.get(_argv)._depth);   }
    if(!_argv && !done){  self.consumer(argv, args);  }
    else if( !done ){  self.emit('next', argv);  }
    return Boolean(_argv || !done || false);
  }
