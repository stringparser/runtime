...
function Runtime(name, opts){
  ...
  // if not make input and output through streams
  var argv = util.argv(process.argv);
  this.repl((argv.repl && argv.repl !== 'false') || false, argv);
...
Runtime.prototype.repl = function(start, opts){
  ...
  Interface.call(this,
    util.type(opts.input).match(/stream/) || util.through.obj(),
    util.type(opts.output).match(/stream/) || util.through.obj(),
