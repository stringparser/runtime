...
function Stack(args, app){
  ...
  this.console = function(error, next){
    if(error instanceof Error){
      this.console.error.call(this, error, next);
    } else if(this.console.log){
      this.console.log.call(this, next);
    }
  };

  // logging
  this.console.log = root.log && (util.type(this.log).function
    || util.type(root.log).function
    || defaultLogger
  );

  // error
  this.console.error = util.type(this.error).function
