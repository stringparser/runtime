...
function Stack(args, app){
  ...
  if(!app){
    util.defineFrozenProperty(this, 'path', path);
    return this;
  }

  var node, firstStem = argv[0].path || argv[0];
  if(typeof firstStem === 'string'){ node = app.get(path); }

  var parent = app.get(node && node.parent);
  opt.log = app.store.log && (util.type(opt.log).function
    || util.type(parent && parent.log || app.store.log).function
    || defaultLogger);
...
// --
//
Stack.prototype.onHandleCall = function(next){
...
// --
//
Stack.prototype.onHandleEnd = function(next){
  if(this.log){ this.log(next); }
};
