...
function Stack(args, app){
  ...
  this.onLog = app.store.log && (util.type(opt.onLog).function
    || util.type(this.onLog || parent.onLog).function || defaultLogger);
  ...
  Object.keys(opt).forEach(function(prop){
    ...
    var handle = util.type(opt[prop]).function || parent[prop];
    if(!handle){ return; }
